...
def is_failed(direction, stone, pos_of_stones, pos_of_switches, pos_of_walls, width, height):
    ...
    if stone not in pos_of_switches:
        ...
        if direction == 'U':
            ...
            for index in [0, 3, 4, 7]:
                new_board = [board[i] for i in all_patterns[index]]
                if new_board[1] in pos_of_walls and new_board[5] in pos_of_walls: 
                    ...
                    return True
                elif new_board[0] in pos_of_walls and new_board[1] in pos_of_stones and new_board[5] in pos_of_walls: 
                    return True
        elif direction == 'R':
            if stone[1] == width - 1:
                no_switch = True
            ...
            if stone[0] == height - 1:
                no_switch = True
                for switch in pos_of_switches:
                    if switch[0] == height - 1 and switch not in pos_of_stones:
                        no_switch = False
                        break
                if no_switch:
                    return True
...
def heuristic(pos_of_stones, pos_of_switches):
    ...
    pos_of_stones = set(pos[0:2] for pos in pos_of_stones)
    ...
    for stone in list_pos_of_stones:
        for switch in list_pos_of_switches:
            total_distance += abs(stone[0] - switch[0]) + abs(stone[1] - switch[1])
