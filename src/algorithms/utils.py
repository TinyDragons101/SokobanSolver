...
def update_state(action, pos_of_player, pos_of_stones):
    ...
    if action[-1].isupper():
        cur_pos_of_stone = [pos for pos in pos_of_stones if pos[0:2] == new_pos_of_player]
        pos_of_stones.remove(cur_pos_of_stone[0])
        weight_push = cur_pos_of_stone[0][2]
        pos_of_stones.append([x_player + 2 * action[0], y_player + 2 * action[1], weight_push])
